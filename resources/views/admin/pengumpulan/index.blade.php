                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if (Auth::user()->is_guru || Auth::id() === $pengumpulan->user_id)
                                <a href="{{ route('pengumpulan.download', $pengumpulan->id) }}" class="btn btn-success btn-sm">Download</a>
                                @if (Auth::user()->is_guru)
                                    <a href="{{ route('pengumpulan.nilai', $pengumpulan->id) }}" class="btn btn-warning btn-sm">Berikan Nilai</a>
                                @endif
                            @endif
                            @if (Auth::id() === $pengumpulan->user_id)
                                <form action="{{ route('pengumpulan.destroy', $pengumpulan->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
